...
| `mybuddy` / `mybuddy.exe` | Linux / Windows | SDL3 **buddy editor**: build and edit alternative co-op buddies (`BUDDYDEF` records) *and* the WAD around them. Buddy side: add/duplicate/delete records, every key edited with the engine's own value sets (ability / colour / attack pickers), rows colour-coded by how much of a key reaches the game today (live / pending / retired) with a per-row help line, **Marine baseline** to seed the stats from the built-in Marine, and a live preview of the exact text written back. WAD side: the lump directory with **Import file / Export lump / Rename lump / Delete lump** — imported art whose name matches the buddy's sprite base is placed *inside* the sprite namespace (creating `SS_START`/`SS_END` if the WAD has none), which is the usual reason a modder's sprites stay invisible. A **sprite preview** renders frame A of the buddy's art from the WAD — Doom patch *or* PNG — falling back to the IWAD so `sprite BOSS` previews with no art of your own. `mybuddy --check <wad>` prints the same information headlessly (buddies, stats, art, problems) for scripts. Build: Linux `tools/build_mybuddy.sh`; Windows `tools/build_mybuddy_win.sh` or `build_all_win.bat`. See **`../docs/BUDDY_MODDING.md`**. |
