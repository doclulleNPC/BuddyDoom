...
int		key_inv_left;		// (J) artifact inventory: select prev / next / use / drop
int		key_inv_right;		//   (defaults '[' ']' Enter 'd'; see m_misc.c defaults[])
int		key_inv_use;
int		key_inv_drop;
int		key_nextweapon;		// default: mouse wheel up
int		key_prevweapon;		// default: mouse wheel down
int		key_jump;		// MOD: jump (default: space)
int		key_turret;		// MOD: deploy sentry turret (default: 'q')
...
{ 
    ...
    // run is now a latched toggle (autorun), flipped by the run key.
    speed = autorun ? 1 : 0;
...
boolean G_Responder (event_t* ev)
{
	...
	if (ev->data1 == key_speed)	// run key toggles always-run
	{
	    autorun = !autorun;
...
{ 
    ...
    sprintf (name,SAVEGAMENAME"%d.dsg",savegameslot);	// always ID0/ (dstrings.h) -- no c:\doomdata
